...
const modelSync = async () => {
    ...
    await Post_Type.sync({ alter: true });
    await Post.sync({ alter: true });
    await Follow.sync({ alter: true });
    await Comment.sync({ alter: true });
    await Vote_Type.sync({ alter: true });
    await Vote_Post.sync({ alter: true });
    await Vote_Comment.sync({ alter: true });
    await Tag.sync({ alter: true });
    await Image_Tag.sync({ alter: true });
    await Post_Tag.sync({ alter: true });
    await Image_Post.sync({ alter: true });
    await List_Save.sync({ alter: true });
    await Item_Save.sync({ alter: true });
